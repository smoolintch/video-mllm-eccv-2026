...
#     --model llava_vid \
#     --model_args pretrained=/data/models/LLM/LLaVA-NeXT-Video-7B-Qwen2,conv_template=chatml_direct,video_decode_backend=decord,max_frames_num=64,overwrite=False,use_topk=True \
...
CUDA_VISIBLE_DEVICES=1,2,3 accelerate launch --num_processes 3 --main_process_port 12345 -m lmms_eval \
    --model llava_vid \
    --model_args pretrained=/data/models/LLM/LLaVA-NeXT-Video-7B-Qwen2,conv_template=chatml_direct,video_decode_backend=decord,max_frames_num=64,overwrite=False,use_topk=True \
    --tasks longvideobench_val_v \
    --batch_size 1 \
